...
function buildConceptControlsHtml(isConceptBookmarked: boolean, detailsBodyHtml: string | null) {
  ...
  return `<div class="pm-inline-concept-actions" data-annotation-ignore="true"><button type="button" data-concept-bookmark="true" aria-pressed="${isConceptBookmarked ? 'true' : 'false'}">${isConceptBookmarked ? 'Bookmarked' : 'Bookmark concept'}</button>${conceptDetailsHtml}</div>`;
}
