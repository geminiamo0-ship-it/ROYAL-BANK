...
export async function POST(request: Request) {
  ...
  const ipHmac = hmacSignal(riskHmacSecret, getRequestIp(request));
  const userAgentHmac = hmacSignal(riskHmacSecret, normalizeUserAgent(request));
  ...
  try {
    upstream = await fetch(`${supabaseUrl}/rest/v1/rpc/${rpcName}`, {
      ...
      headers: {
        ...
        'x-royal-gateway-key-id': gatewayKeyId,
        'x-royal-gateway-key': gatewayKey,
        'x-royal-ip-hmac': ipHmac,
        'x-royal-ua-hmac': userAgentHmac,
