...
export default async function AdminUsersPage({ searchParams }: { searchParams: Promise<{ q?: string }> }) {
  ...
  const grantRowsByUser = new Map<string, GrantRow[]>();
  for (const grant of grants) {
    grantRowsByUser.set(grant.user_id, [...(grantRowsByUser.get(grant.user_id) || []), grant]);
  }
    ...
    <div className="mx-auto max-w-7xl space-y-6 pb-16 text-xs sm:text-sm">
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        <div>
          ...
          <p className="text-xs text-slate-500">Live profile, login, account status, and entitlement-ledger data.</p>
        </div>
      ...
      <div className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-xs dark:border-slate-800 dark:bg-slate-900">
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            <thead><tr className="border-b border-slate-200 bg-slate-50 text-[11px] font-bold uppercase tracking-wider text-slate-500 dark:border-slate-800 dark:bg-slate-800/60"><th className="px-4 py-3">Account</th><th className="px-4 py-3">Role</th><th className="px-4 py-3">Entitlement rows</th><th className="px-4 py-3">Last login</th><th className="px-4 py-3">Status</th><th className="px-4 py-3 text-right">Action</th></tr></thead>
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {visibleProfiles.map((profile) => {
                const userGrants = grantRowsByUser.get(profile.id) || [];
                const global = userGrants.some((grant) => grant.scope_type === 'global');
                const pathwayCount = userGrants.filter((grant) => grant.scope_type === 'pathway').length;
                const bankCount = userGrants.filter((grant) => grant.scope_type === 'bank').length;
                ...
                return (
                  <tr key={profile.id} className="align-top hover:bg-slate-50/50 dark:hover:bg-slate-800/30">
                    ...
                    <td className="px-4 py-3">{userGrants.length === 0 ? <span className="text-slate-400">No non-revoked grant rows</span> : <div className="space-y-1 text-[11px] text-slate-700 dark:text-slate-300">{global && <p>Global grant</p>}{pathwayCount > 0 && <p>{pathwayCount} pathway grant{pathwayCount === 1 ? '' : 's'}</p>}{bankCount > 0 && <p>{bankCount} bank grant{bankCount === 1 ? '' : 's'}</p>}<p className="text-[10px] text-slate-400">Dates remain authoritative for whether each row is currently effective.</p></div>}</td>
