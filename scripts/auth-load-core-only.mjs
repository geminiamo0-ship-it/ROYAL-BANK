...
async function gateway(cookie, action, args, token) {
  const headers = {
    ...
    cookie,
    'user-agent': 'RoyalBank-Authorized-LoadTest/3.0-core-only',
  };
...
console.log(`Starting isolated exam-core diagnostic with ${users.length} concurrent authenticated users.`);
...
for (const row of coreResults.filter((r) => !r.ok)) {
  failures[row.failedStep || 'unknown'] = (failures[row.failedStep || 'unknown'] || 0) + 1;
}
console.log('CORE_ONLY_DIAGNOSTIC ' + JSON.stringify({
...
for (const row of coreResults.filter((r) => !r.ok)) {
  console.log(`CORE_ONLY_FAILURE ${row.email} step=${row.failedStep} statuses=${JSON.stringify(row.statuses)} detail=${JSON.stringify(row.detail || '')}`);
}
