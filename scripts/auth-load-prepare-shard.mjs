...
const requestedCount = Number(process.env.LOAD_USER_COUNT || 100);
const userCount = Math.max(1, Math.min(Number.isFinite(requestedCount) ? Math.floor(requestedCount) : 100, 100));
const requestedPacingMs = Number(process.env.LOAD_AUTH_PACING_MS || 1600);
const authPacingMs = Math.max(0, Math.min(Number.isFinite(requestedPacingMs) ? Math.floor(requestedPacingMs) : 1600, 10000));
const shard = String(process.env.LOAD_SHARD || '0').replace(/[^0-9A-Za-z_-]/g, '').slice(0, 16) || '0';
...
for (let i = 1; i <= userCount; i += 1) {
  const email = `royal-load-1000-${runTag}-${String(i).padStart(3, '0')}@load.invalid`;
  const created = await admin.auth.admin.createUser({
    email,
    password,
    email_confirm: true,
    user_metadata: { full_name: `Royal Load 1000 S${shard} U${i}` },
  });
...
console.log(`Prepared ${rows.length} isolated authenticated users in shard ${shard}.`);
