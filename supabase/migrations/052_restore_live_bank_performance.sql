-- Restore the live bank performance RPCs from the canonical migration.
-- DEV bootstrap left get_question_bank_performance with an incomplete PL/pgSQL body,
-- causing /bank/:id to fail with a server error before normal access handling.
