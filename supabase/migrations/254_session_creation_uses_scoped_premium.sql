-- Session creation must use the same canonical premium-access decision as the
-- rest of the application. Scoped global/pathway/bank grants must not be treated
-- as trial users when applying per-block question limits.
...
'Creates and locks an exam block atomically after canonical scoped access, trial quota, state and filter validation. Scoped premium grants receive the normal 70-question ceiling.';
