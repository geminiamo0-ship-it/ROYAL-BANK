CREATE OR REPLACE FUNCTION public.can_read_locked_session(p_session_id UUID)
RETURNS BOOLEAN
LANGUAGE sql
STABLE
SECURITY DEFINER
...
CREATE POLICY "Users read accessible locked session questions"
    ON public.test_session_questions FOR SELECT TO authenticated
    USING (public.is_active_user() AND public.can_read_locked_session(test_session_id));
