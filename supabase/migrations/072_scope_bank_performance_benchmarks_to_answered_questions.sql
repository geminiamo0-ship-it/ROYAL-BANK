-- Avoid rebuilding benchmark metadata for every question in a bank on a cold/dirty
-- performance request. Only answered questions contribute to accuracy, peer,
-- difficulty-adjusted score, category score, or percentile math, so drive the
-- option/benchmark aggregate from answered question state instead of the full bank.
-- State totals (total/flagged/suspended) still use the canonical full-bank state RPC.

CREATE OR REPLACE FUNCTION public.compute_question_bank_performance(p_bank_id bigint)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
    ...
    answered_states AS MATERIALIZED (
        SELECT question_id, answer_state
        FROM states
        WHERE answer_state IS NOT NULL
    ),
    question_reference AS (
        ...
        FROM answered_states answered_state
        JOIN public.questions q ON q.id = answered_state.question_id
    ...
    answered AS (
        SELECT
            ...
            qr.peer_probability,
            (answered_state.answer_state = 'correct') AS is_correct
        FROM answered_states answered_state
        JOIN question_reference qr ON qr.question_id = answered_state.question_id
    ),
    overall AS (
        SELECT
...
-- Existing wrapper privileges stay unchanged by CREATE OR REPLACE, but keep the
-- internal compute surface explicitly non-client-executable.
